...
describe('anchorAction ⇄ REQUIREMENT_EVENT_KINDS (no drift)', () => {
  ...
  it('the authoritative list is non-empty and matches the count (PIVOT 10 + OVERHAUL steer_directive/rework_requested/manifest_ready = 13 + INTAKE scout_result = 14)', () => {
    expect(REQUIREMENT_EVENT_KINDS.length).toBe(14);
  });
});
