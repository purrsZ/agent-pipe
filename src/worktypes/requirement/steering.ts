...
export function composeSteerPrompt(
  ...
  lines.push(
    ...
    '2. 用户要求产出与本需求相关的**衍生文本产物**（测试用例、自测清单、发布说明、风险清单等）属于你分内事——',
    '   直接在回复主体完整产出，不要上报也不要推给工人；内容长就完整写（你的报告会作为回执存档，卡片截断不影响存档）。',
    '3. 在报告**最末尾**输出且仅输出一个 ```steer 代码块，承载结构化动作：',
    '```steer',
    '{ "action": "none | redo_reconcile | rework | raise_human",',
    '  "repos": ["受影响仓绝对路径（仅 rework 填，必须取自上面的涉及仓库清单）"],',
    '  "note": "给工人的定向施工指令；raise_human 时必须写清：要人裁决什么 + 你的建议方案与理由" }',
    '```',
    '选择规则：只是答疑 / 确认 / 产出衍生文本 → none；用户对跨仓契约或拆解结论提出修改 → redo_reconcile；',
    '用户要求改某仓的实现方向，或给某仓**新增小型施工任务**（补测试代码、补文档等）且不改变需求范围 → rework',
    '（repos 只填清单内的仓，note 写清这是新增施工指令、在现有产出基础上做）；拿不准、或用户要求超出当前需求',
    '范围 → raise_human（疑则上报——note 必须包含：要人裁决什么 + **你的建议方案与理由**，人会参考你的建议来裁决）。',
  );
  return lines.join('\n');
}
