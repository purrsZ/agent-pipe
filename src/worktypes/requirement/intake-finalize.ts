...
// 立项收尾 effect（intake-phase）：立项 gate 通过、进「理解」时由 worktype 发起（enterPhase）。两件事
// 都是从立项填项事件历史纯 fold 出来的派生产物（recovery:'rerun'，崩溃后重跑幂等）：
//   1) 落「立项书」intake/intake.md —— 合同 phase 的 spec-design run 据它装配（PRD 摘要/验收/UI/边界/
//      多仓），而非裸标题硬考古；
//   2) 把立项收齐的仓库提升为 workitem.repos —— worker 拆分 / spec-design 的 repo 候选都读
//      workitem.repos，而 /req 不再带 --repo（仓库走立项收齐）。effect 不能直接改 workitem 字段，
//      故走中性 repos_set 容器事件（reducer.containerTransition 落库）。
