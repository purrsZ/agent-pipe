...
export function composeWorkerPrompt(input: WorkerPromptInput): string {
  const lines: string[] = [
    ...
    // VERIFY V1（#5+R5 交付完整性）：交付链假设 worker 会提交，但此前没人告诉它 → 改动全留 worktree 未提交，
    // 交付清单读已提交 diff 报「(无改动)」、终态 GC 7 天后连未提交产出一起删。这里把提交契约钉进 prompt：
    // 你的分支就是唯一交付物，未提交 = 隐形 + 最终被清理丢弃。禁止 push / 禁止建 MR（出站发布需人手拍板）。
    '',
    '# 提交纪律（唯一交付物，务必执行）',
    '完成后必须把全部改动 `git commit` 到当前分支（可多次小提交，把工作留在分支上即可）。',
    '**你的分支就是唯一交付物**——未提交的改动会在交付清单里隐形，并最终被自动清理丢弃。',
    '**禁止 `git push`、禁止建 MR/PR**：推送与发布由人手在拍板后进行，你只负责把活提交到本地分支。',
    '',
